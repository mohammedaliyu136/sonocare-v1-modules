...
class AmbulanceForm extends StatefulWidget {
  const AmbulanceForm({Key? key}) : super(key: key);
...
class _AmbulanceFormState extends State<AmbulanceForm> {
  ...
  void _register(AuthController authController) async {
    ...
    if (_name.isEmpty) {
      ...
      AmbulanceSignUpBody ambulanceSignUpBody = AmbulanceSignUpBody(name: _name, email: _email, phone: _number, password: _password, serviceID: _serviceID, dob: _dob, lgaID: _lgaID, fee: _fee, stateID: _stateID);
      authController.registration(ambulanceSignUpBody:ambulanceSignUpBody, accountType: 'ambulance').then((status) async {
        if (status.isSuccess) {
        }else {
    ...
    Get.toNamed('/otp', parameters: {'email':_email, 'password':_password,'accountType':'ambulance'});
  }
}
