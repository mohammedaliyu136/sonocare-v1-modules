...
class _LabFormState extends State<LabForm> {
  ...
  void _register(AuthController authController) async {
    ...
    if (_name.isEmpty) {
      ...
      LabSignUpBody labSignUpBody = LabSignUpBody(name: _name, email: _email, phone: _number, password: _password, serviceID: _serviceID);
      authController.registration(labSignUpBody:labSignUpBody, accountType:'lab').then((status) async {
        if (status.isSuccess) {
        }else {
    ...
    Get.toNamed('/otp', parameters: {'email':_email, 'password':_password,'accountType':'lab'});
  }
}
