...
class _DoctorFormState extends State<DoctorForm> {
  ...
  @override
  void initState() {
    Get.find<AuthController>().getDoctorType();
                    ...
                    Expanded(
                      child: Padding(
                        ...
                        child: Column(
                          ...
                          crossAxisAlignment: CrossAxisAlignment.start,
                          children: [
                            Text('doctor_type'.tr, textAlign: TextAlign.start, style: const TextStyle(fontSize: 14, fontWeight: FontWeight.w400, color: Colors.white),),
                            if(!authController.loadingDoctorType)DropdownButton<DoctorType>(
                              isExpanded: true,
                              dropdownColor: ColorResources.COLOR_PURPLE_DEEP,
                              underline: Container(color: Colors.transparent),
                              items: authController.doctorTypeList.map((DoctorType dropDownStringItem) {
                                return DropdownMenuItem<DoctorType>(
                                  value: dropDownStringItem,
                                  child: Text(dropDownStringItem.title, style: const TextStyle(color: Colors.white),),
                                );
                              }).toList(),
                              onChanged: (value){
                                selectedDoctorType = value!;
                                _doctorTypeController.text = selectedDoctorType.title;
                              },
                              value: selectedDoctorType.title==''?authController.doctorTypeList[0]:selectedDoctorType,
                            ),
                            if(authController.loadingDoctorType)Row(children: const [Text('Loading Doctor Type', style: TextStyle(color: ColorResources.COLOR_WHITE),),],),
                          ],
                        ),
                      ),
                    ),
  ...
  void _register(AuthController authController) async {
    ...
    String _doctorType = _doctorTypeController.text.trim();
    ...
    if (_firstName.isEmpty) {
      ...
      DoctorSignUpBody doctorSignUpBody = DoctorSignUpBody(firstName: _firstName, lastName: _lastName, otherName: _otherName, email: _email, phone: _number, password: _password, serviceID: _serviceID, doctorType: _doctorType);
      authController.registration(doctorSignUpBody:doctorSignUpBody, accountType: 'doctor').then((status) async {
        if (status.isSuccess) {
        }else {
    ...
    */
    Get.toNamed('/otp', parameters: {'email':_email, 'password':_password,'accountType':'doctor'});
  }
}
